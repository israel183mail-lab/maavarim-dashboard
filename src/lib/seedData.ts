...
// Shared by prisma/seed.ts (CLI: `npm run seed`) and the one-time
// /api/setup/seed route, so local and hosted setup stay identical.
export async function runSeed(prisma: PrismaClient) {
  ...
  async function seedIndividualSteps(
  ...
  return { coordinatorEmails: coordinatorNames.map(([, e]) => e) };
}
